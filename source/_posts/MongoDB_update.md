...
- $inc : $set과 비슷하지만, 숫자를 증감하기 위해 사용. int, long, double, decimal 타입 값에만 사용 가능
...
- $each : $push에 $each 제한자를 사용하면 작업 한 번으로 값을 여러개 추가할 수 있음.
...
- 고유한 값을 여러개 추가하려면 $addToSet/$each 조합을 활용해야 함. $ne/$push조합으로는 할 수 없음.
